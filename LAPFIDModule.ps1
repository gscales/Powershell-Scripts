...
function Get-LAPFIDItemsFromFolder {
    param( 
        [Parameter(Position = 0, Mandatory = $true)] [string]$MailboxName,
        ...
        [Parameter(Position = 4, Mandatory = $false)] [switch]$recoverableItemsDeletions,
        [Parameter(Position = 5, Mandatory = $false)] [switch]$recoverableItemsPurges,
        [Parameter(Position = 6, Mandatory = $false)] [switch]$deletedItems,
        [Parameter(Position = 7, Mandatory = $false)] [String]$QueryString,
        [Parameter(Position = 8, Mandatory = $false)] [string]$FolderPath,
        [Parameter(Position = 9, Mandatory = $false)] [Int]$MaxCount = 0,
        [Parameter(Position = 10, Mandatory = $false)] [switch]$Archive,
        [Parameter(Position = 11, Mandatory = $false)] [switch]$basicAuth,
        [Parameter(Position = 12, Mandatory = $false)] [System.Management.Automation.PSCredential]$Credentials  
    ...
    Process {
        ...
        $folders = Get-FolderFromPath -MailboxName $MailboxName -FolderPath $FolderPath -service $service -Recurse -RootFolder -Archive:$false
        if ($Archive.IsPresent) {
            $ArchiveFolders = Get-FolderFromPath -MailboxName $MailboxName -RootFolder -Recurse -Archive -service $service 
        ...
        foreach ($fld in $folders) {
            $laFid = $fld.PR_ENTRYID.substring(44, 44)
            Write-Verbose $laFid
            $fldIndex.Add($laFid, $fld);
        }
        $ArchiveFolderId = $null;
        if ($recoverableItemsDeletions.IsPresent) {
            ...
            $ArchiveFolderId = new-object Microsoft.Exchange.WebServices.Data.FolderId([Microsoft.Exchange.WebServices.Data.WellKnownFolderName]::ArchiveRecoverableItemsDeletions, $MailboxName)
        }
        else {
            if ($recoverableItemsPurges.IsPresent) {
        ...
        $Items = Invoke-GenericFolderItemEnum -Folder $dlFolder -MaxCount $MaxCount -fldIndex $fldIndex -QueryString $QueryString
        if ($Archive.IsPresent) {
            $dlFolder = [Microsoft.Exchange.WebServices.Data.Folder]::Bind($service, $ArchiveFolderId)
            $ArchiveItems = Invoke-GenericFolderItemEnum -Folder $dlFolder -MaxCount $MaxCount -fldIndex $fldIndex
...
function Invoke-GenericFolderItemEnum {
    ...
    Process {
        ...
        $ItemPropset.Add($LastActiveParentEntryId)
        ...
        do { 
            ...
            foreach ($Item in $fiItems.Items) { 
                ...
                if ($Okay) {
                    $LastActiveParentEntryIdval = $null
                    if ($Item.TryGetProperty($LastActiveParentEntryId, [ref]$LastActiveParentEntryIdval)) {
                        $HexVal = [System.BitConverter]::ToString($LastActiveParentEntryIdval)                                
                        $Item | Add-Member -Name "LastActiveParentEntryId" -Value $HexVal.Replace("-", "") -MemberType NoteProperty
