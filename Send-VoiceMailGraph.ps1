function Send-VoiceMail {
    ...
    Begin {        
        ...
        $prop = "" | Select Id,DataType,PropertyType,Value
        $prop.id = "0x001A"
        $prop.DataType = "String"
        $prop.PropertyType = "Tagged"
        $prop.Value = "IPM.Note.Microsoft.Voicemail.UM.CA"
        $exProp = @()
        $exProp += $prop
        $NewMessage = Get-MessageJSONFormat -Subject $VoiceMailSuject -Body $BodyHtml.Replace("`"","\`"") -SenderEmailAddress $SenderAddress -Attachments @($Mp3FileName) -ToRecipients @($ToRecp) -SaveToSentItems "true" -SendMail -ExPropList $exProp
...
function Get-VoiceMail {
    ...
    Begin {        
        ...
        $Result =  Invoke-RestMethod -Headers $Header -Uri ("https://graph.microsoft.com/v1.0/users('" + $MailboxName + "')/mailFolders/voicemail/messages?$expand=SingleValueExtendedProperties($filter=PropertyId%20eq%20'Integer%20{00020328-0000-0000-C000-000000000046}%20Id%200x6801'%20or%20PropertyId%20eq%20'String%20{00020386-0000-0000-C000-000000000046}%20Name%20X-VoiceMessageConfidenceLevel'%20or%20PropertyId%20eq%20'String%20{00020386-0000-0000-C000-000000000046}%20Name%20X-VoiceMessageTranscription')&`$top=100&`$select=Subject,From,Body,IsRead,Id,ReceivedDateTime")
        if ($Result.value -ne $null) {
             foreach ($Message in $Result.value ) {
                 write-output $Message
             }
