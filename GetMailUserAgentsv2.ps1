...
function Get-MailUserAgents{
    param(
		...
		[Parameter(Position=4, Mandatory=$false)] [Int32]$days,
		[Parameter(Position=5, Mandatory=$false)] [Switch]$Inbox
    )  
 	Begin
	{
		...
		do{    
			...
			{
				$qDate = [DateTime]::Now.AddDays(-$days)
				$fiItems = $service.FindItems($folder.Id,"Sent:>" + $qDate.ToString("yyyy-MM-dd"), $ivItemView) 
			}
				...
				if($Item.TryGetProperty($ClientInfo,[ref]$ClientInfoVal)){
					...
					if([string]::IsNullOrEmpty($ClientInfoVal)){
						...
						if($ClientInfoVal.Contains("Client=")){
							$parsedVal = $ClientInfoVal.Substring(7)
							$ClientVal = ""
							$PrimaryVal = ""
							$SubTypeVal = ""
							$SubType = $false
							$inBracket = $false
						    for($valcnt=0;$valcnt -lt $parsedVal.length;$valcnt++)
							{								
								switch($parsedVal.SubString($valcnt,1))
								{
									";" {if($inBracket -eq $false) 
									     {
											 $SubType = $true
										 }
									     else
										 {
									   		  $ClientVal += $parsedVal.SubString($valcnt,1)
											  if(!$SubType)
											  {
											  	$PrimaryVal += $parsedVal.SubString($valcnt,1)
											  }
											  else
											  {
											     $SubTypeVal += $parsedVal.SubString($valcnt,1)
											  }
										 }
										}
									"(" {$ClientVal += $parsedVal.SubString($valcnt,1)
							...
							if($reportHash.ContainsKey($ClientVal))
							{
								$reportHash[$ClientVal].ItemCount ++
								$reportHash[$ClientVal].ItemSize += $Item.Size
							}
							else
							{
								$rptobj = "" | select ClientType,SubType,ItemCount,ItemSize
								$rptobj.ClientType = $PrimaryVal
								$rptobj.SubType = $SubTypeVal
								$rptobj.ItemCount = 1
								$rptobj.ItemSize = $Item.Size
								$reportHash.Add($ClientVal,$rptobj)
							}
		...
		$reportHash.Values | Export-Csv -NoTypeInformation -Path c:\temp\clientAgentReport.csv
	}
