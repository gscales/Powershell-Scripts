...
function Invoke-GenerateIMReportbyDate {
    param(
        [parameter(ValueFromPipeline)][PsObject[]]$rptCollection  
    )
    Process {
        $rptHash = @{}
        write-verbose "1"
        foreach ($rptObject in $rptCollection) {
            write-verbose "3"
        }
        # $rptCollection | Select-Object @{n="Date";e={$_.FirstMessage.Date.ToString("yyyy-MM-dd")}},Subject, Sender, Recipients, FirstMessage, LastMessage, MessageCount,CallCount | Sort-Object Date -Descending | foreach-object{
        #     if($rptHash.ContainsKey($_.Date)){
        #         $rptObject = $rptHash[$_.Date]
        #     }else{
        #         $rptObject = "" | Select Date,Sent,Received,PeopleMessaged,TotalCount
        ...
        #     }
        # }
        # Write-Output $rptHash.Values | Sort-Object Date -Descending
    }
}
