...
function Get-TeamsChatMessages{
    param( 
        ...
        [Parameter(Position = 5, Mandatory = $false)] [switch]$basicAuth,
        [Parameter(Position = 6, Mandatory = $false)] [System.Management.Automation.PSCredential]$Credentials,
        [Parameter(Position = 7, Mandatory = $false)] [DateTime]$startdatetime = (Get-Date).AddDays(-30),
        [Parameter(Position = 8, Mandatory = $false)] [datetime]$enddatetime = (Get-Date)
    ...
    Process {
        ...
        Invoke-ScanFolderItems -Folder $TeamsMessagesDataFolder -MailboxName $MailboxName -startdatetime $startdatetime -enddatetime $enddatetime
...
function Invoke-ScanFolderItems { 
    ...
    Begin { 
        ...
        $DetailPropSet.RequestedBodyType = [Microsoft.Exchange.WebServices.Data.BodyType]::Text
        $DetailPropSet.Add([Microsoft.Exchange.WebServices.Data.ItemSchema]::Preview)
        $ivItemView.PropertySet = $ItemPropset
        $fiItems = $null    
        do {
            try {
                $error.Clear()
                $fiItems = $Folder.Service.FindItems($Folder.Id,$sfCollection, $ivItemView)   
            }
            catch {
