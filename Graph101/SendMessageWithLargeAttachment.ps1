...
function Invoke-SendMessageWithLargeAttachment{
    ...
    param (
        ...
        [String]
        $AccessToken,
        [Parameter(Position = 8, Mandatory = $true)] [string]$filePath,
        [Parameter(Position = 9, Mandatory = $false)] [Int32]$uploadChunkSize=4000,  
    ...
    process {
        ...
        $Draft = New-DraftMessage -AccessToken $AccessToken -MailboxName $MailboxName -Subject $Subject -Body $Body.Replace("`"","\`"") -From $From -To $To -Cc $CC -bcc $BCC 
        $UploadSession = "https://graph.microsoft.com/v1.0/users('" + $MailboxName + "')/messages/" + $Draft.id + "/attachments/createUploadSession"
...
"@
        ...
        $Results = (Invoke-RestMethod -Method POST -Uri $UploadSession -UserAgent "GraphBasicsPs101" -Headers $headers -Body $File -ContentType "application/JSON") 
        ...
        do {            
            ...
            if($FileChunkByteCount -gt 0){
                ...
                $Results = (Invoke-RestMethod -Method Put -Uri $UploadUrl -UserAgent "GraphBasicsPs101" -Headers $headers -Body $binaryContent.ReadAsByteArrayAsync().Result -ContentType "application/octet-stream") 
            }          
        ...
        $SendURL = "https://graph.microsoft.com/v1.0/users('" + $MailboxName + "')/messages/" + $Draft.id + "/send"
        $SendResult = (Invoke-RestMethod -Method POST -Uri $SendURL -UserAgent "GraphBasicsPs101" -Headers $headers -Body "" -ContentType "application/JSON") 
        return $SendResult
    }
}
