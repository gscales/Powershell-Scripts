...
function Get-LastEmail{
    ...
    process {
        ...
        if($ExternalSender.IsPresent){
            if($RequestURL.Contains("filter=")){
                $RequestURL += " AND "
            }else{
                $RequestURL += "&`$filter="
            }
            $RequestURL +="singleValueExtendedProperties/Any(ep:+ep/id+eq+'Boolean+{41F28F13-83F4-4114-A584-EEDB5A6B0BFF}+Name+IsExternalSender'+and+cast(ep/value,+Edm.Boolean)+eq+true)"
        }
        if($InternalSender.IsPresent){
            if($RequestURL.Contains("filter=")){
                $RequestURL += " AND "
            }else{
                $RequestURL += "&`$filter="
            }
            $RequestURL +="singleValueExtendedProperties/Any(ep:+ep/id+eq+'Boolean+{41F28F13-83F4-4114-A584-EEDB5A6B0BFF}+Name+IsExternalSender'+and+cast(ep/value,+Edm.Boolean)+ne+true)"
        }
        $MessageEnumCount =0;
        do {
